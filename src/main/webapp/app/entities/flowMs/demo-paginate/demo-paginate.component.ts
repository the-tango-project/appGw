...
export default defineComponent({
  ...
  setup() {
    ...
    const demoPaginates: Ref<IDemoPaginate[]> = ref([]);
    ...
    const retrieveDemoPaginates = async () => {
      ...
      try {
        ...
        const res = await demoPaginateService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.['link']);
        demoPaginates.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    onMounted(async () => {
      await retrieveDemoPaginates();
    });

    const removeId: Ref<string> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IDemoPaginate) => {
    ...
    const removeDemoPaginate = async () => {
      try {
        await demoPaginateService().delete(removeId.value);
        const message = t$('flowMsApp.flowMsDemoPaginate.deleted', { param: removeId.value }).toString();
    ...
    watch([demoPaginates, page], async ([data, page], [_prevData, prevPage]) => {
